...
class ResetPasswordForm(forms.Form):
    ...
    def clean_email(self):
        # My spinnof:
        #if profiles.models.Profile.objects.filter(email__iexact=self.cleaned_data["email"]).count() == 0:
        #    raise form.ValidationError(_("No user with this email exists"))
        #return self.cleaned_data["email"]
        ...
        # Considering mail activations
        if mail.models.EmailConfirmation.objects.filter(email__iexact=self.cleaned_data["email"], confirmed=True).count() == 0:
            raise forms.ValidationError(_("Email address not confirmed for any user account"))
        return self.cleaned_data["email"]

    def save(self):
        import random
        for profile in profiles.models.Profile.objects.filter(email__iexact=self.cleaned_data["email"]):
            user = profile.user

            query =  PasswordReset.objects.filter(user=user, reset=False)
            if query.count():
                temp_key = query[0].temp_key
            else:
                ...
                while PasswordReset.objects.filter(temp_key=temp_key, reset=False).count():
                    temp_key = sha_constructor("%s%s%s%d" % (
                        settings.SECRET_KEY,
                        profile.email,
                        settings.SECRET_KEY,
                        random.randint(1, 1<<32)
                    )).hexdigest()
                
                # save it to the password reset model
                password_reset = PasswordReset(user=user, temp_key=temp_key, reset=False)
                password_reset.save()
            ...
            mail.utils.mail(profile.email, subject, message)
            #send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, [profile.email], priority="high")
        return self.cleaned_data["email"]
...
class ResetPasswordKeyForm(forms.Form):
    ...
    def save(self):
        print self.cleaned_data.get('temp_key')
